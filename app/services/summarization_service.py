...
class SummarizationService:
    ...
    def summarize(self, db: Session, payload: SummarizeRequest) -> SummarizeResponse:
        ...
        if payload.persist:
            for m in payload.messages:
                message_repository.save(db, Message(
                    ...
                    sender=m.sender,
                    content_encrypted=encrypt(m.content, aad=str(payload.user_id)),
                ))

        audit_service.record(
            db, user_id=payload.user_id, action="MESSAGES_SUMMARIZED",
